...
dat <- fread(here("data/8200574_NS01DL0009_data.csv")) %>% 
  mutate(
    file_type = if_else(
    year_ast >= start_val, "validation", "calibration")
  )
...
dat %>% 
  ...
  fwrite(
    paste0(sim_path, "/", lake, "/8200574_NS01DL0009_cc.txt"), 
    col.names = FALSE
  )
...
dat %>% 
  ...
  select(-c(file_type, date_ast)) %>% 
  fwrite(
    paste0(sim_path, "/", lake, "/8200574_NS01DL0009_cv.txt"), 
    col.names = FALSE
  )
#sim_folder <- here("simulations")
